...
void IRMutationStrategy::mutate(Module &M, RandomIRBuilder &IB) {
  ...
  const uint64_t MinNumArgs = 0;
  const uint64_t MaxNumArgs = 10;
  while (RS.totalWeight() < MinNumFunctions) {
    Function *F = IB.createFunctionDefinition(M, MinNumArgs, MaxNumArgs);
    RS.sample(F, /*Weight=*/1);
  }
...
void FunctionIRStrategy::mutate(BasicBlock &BB, RandomIRBuilder &IB) {
  ...
  if (!F) {
    /// TODO: Parameterize num of arguments
    const uint64_t MinNumArgs = 0;
    const uint64_t MaxNumArgs = 5;
    F = IB.createFunctionDeclaration(*M, MinNumArgs, MaxNumArgs);
  }
